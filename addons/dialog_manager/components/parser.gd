...
func _init() -> void:
	...
	var tokens = {
		DialogConstants.TOKEN_FUNCTION: "^[a-zA-Z_][a-zA-Z_0-9]+\\(",
		DialogConstants.TOKEN_DICTIONARY_REFERENCE: "^[a-zA-Z_][a-zA-Z_0-9]+\\[",
		DialogConstants.TOKEN_PARENS_OPEN: "^\\(",
		DialogConstants.TOKEN_PARENS_CLOSE: "^\\)",
		DialogConstants.TOKEN_BRACKET_OPEN: "^\\[",
		DialogConstants.TOKEN_BRACKET_CLOSE: "^\\]",
		DialogConstants.TOKEN_BRACE_OPEN: "^\\{",
		DialogConstants.TOKEN_BRACE_CLOSE: "^\\}",
		DialogConstants.TOKEN_COLON: "^:",
		DialogConstants.TOKEN_COMPARISON: "^(==|<=|>=|<|>|!=|in )",
		DialogConstants.TOKEN_ASSIGNMENT: "^(\\+=|\\-=|\\*=|/=|=)",
		DialogConstants.TOKEN_NUMBER: "^\\-?\\d+(\\.\\d+)?",
		DialogConstants.TOKEN_OPERATOR: "^(\\+|\\-|\\*|/|%)",
		DialogConstants.TOKEN_COMMA: "^,",
		DialogConstants.TOKEN_DOT: "^\\.",
		DialogConstants.TOKEN_BOOL: "^(true|false)",
		DialogConstants.TOKEN_NOT: "^(not( |$)|!)",
		DialogConstants.TOKEN_AND_OR: "^(and|or)( |$)",
		DialogConstants.TOKEN_STRING: "^\".*?\"",
		DialogConstants.TOKEN_VARIABLE: "^[a-zA-Z_][a-zA-Z_0-9]+",
	}
...
func parse(content: String) -> Dictionary:
	...
	for id in range(0, raw_lines.size()):
		if raw_lines[id].begins_with("~ "):
			...
			if titles.has(title):
				...
				if next_nonempty_line_id != DialogConstants.ID_NULL:
					titles[title] = next_nonempty_line_id
				else:
					titles[title] = DialogConstants.ID_TITLE_HAS_NO_BODY
	...
	for id in range(0, raw_lines.size()):
		...
		var line: Dictionary = {
			next_id = DialogConstants.ID_NULL
		}
		...
		if raw_line.begins_with("- "):
			line["type"] = DialogConstants.TYPE_RESPONSE
			...
			if dialog.has(previous_response_id):
				var previous_response = dialog[previous_response_id]
			...
			line["next_id_after"] = find_next_line_after_responses(id, raw_lines, dialog, parent_stack)

			# If this response has no body then the next id is the next id after
			if not line.has("next_id") or line.get("next_id") == DialogConstants.ID_NULL:
				var next_nonempty_line_id = get_next_nonempty_line_id(id, raw_lines)
				if next_nonempty_line_id != DialogConstants.ID_NULL:
			...
			line["replacements"] = extract_dialog_replacements(line.get("text"))
			...
			# injected as a line of dialog if the player selects it
			var l = line.get("text").replace("\\:", "!ESCAPED_COLON!")
			if ": " in l:
				var first_child: Dictionary = {
					type = DialogConstants.TYPE_DIALOG,
				...
				first_child["character_replacements"] = extract_dialog_replacements(first_child.get("character"))
		...
		# Title
			...
			line["type"] = DialogConstants.TYPE_TITLE
		...
		# Condition
			...
			parent_stack.append(str(id))
			line["type"] = DialogConstants.TYPE_CONDITION
			line["condition"] = extract_condition(raw_line)
			line["next_id_after"] = find_next_line_after_conditions(id, raw_lines, dialog)
		...
		# Mutation
			...
			line["type"] = DialogConstants.TYPE_MUTATION
		...
		# Goto
			...
			line["type"] = DialogConstants.TYPE_GOTO
			line["next_id"] = extract_goto(raw_line, titles)

		# Dialog
		else:
			line["type"] = DialogConstants.TYPE_DIALOG
			var l = raw_line.replace("\\:", "!ESCAPED_COLON!")
			if ": " in l:
				var bits = Array(l.strip_edges().split(": "))
				line["character"] = bits.pop_front()
				# You can use variables in the character's name
				line["character_replacements"] = extract_dialog_replacements(line.get("character"))
		...
		if line.get("next_id") == DialogConstants.ID_NULL:
			...
			if next_nonempty_line_id != DialogConstants.ID_NULL \
				...
				if raw_lines[next_nonempty_line_id.to_int()].strip_edges().begins_with("~ "):
					line["next_id"] = DialogConstants.ID_END_CONVERSATION
				# Otherwise it's a normal line
				else:
					line["next_id"] = next_nonempty_line_id
			# Otherwise, we grab the ID from the parents next ID after children
			elif dialog.has(line.get("parent_id")):
				line["next_id"] = dialog[line.get("parent_id")].get("next_id_after")

		# Check for duplicate transaction keys
		if line.get("type") in [DialogConstants.TYPE_DIALOG, DialogConstants.TYPE_RESPONSE]:
		...
		match line.get("next_id"):
			DialogConstants.ID_ERROR:
				errors.append(error(id, "Unknown title"))
			DialogConstants.ID_TITLE_HAS_NO_BODY:
				errors.append(error(id, "Referenced node has no body"))

		# Line after condition isn't indented once to the right
		if line.get("type") == DialogConstants.TYPE_CONDITION and is_valid_id(line.get("next_id")):
		...
		elif line.get("type") in [DialogConstants.TYPE_TITLE, DialogConstants.TYPE_DIALOG, DialogConstants.TYPE_MUTATION, DialogConstants.TYPE_GOTO] and is_valid_id(line.get("next_id")):
			var next_line = raw_lines[line.get("next_id").to_int()]
		...
		if line.get("type") == DialogConstants.TYPE_UNKNOWN:
...
func is_valid_id(id: String) -> bool:
	return false if id in [DialogConstants.ID_NULL, DialogConstants.ID_ERROR, DialogConstants.ID_END_CONVERSATION] else true
...
func get_next_nonempty_line_id(line_number: int, all_lines: Array) -> String:
	for i in range(line_number + 1, all_lines.size()):
		if not is_line_empty(all_lines[i]):
			return str(i)
	return DialogConstants.ID_NULL
...
func find_next_condition_sibling(line_number: int, all_lines: Array) -> String:
	...
	for i in range(line_number + 1, all_lines.size()):
		...
		if l.begins_with("~ "):
			return DialogConstants.ID_END_CONVERSATION
...
func find_next_line_after_conditions(line_number: int, all_lines: Array, dialog: Dictionary) -> String:
	var line = all_lines[line_number]
	var expected_indent = get_indent(line)
		...
		if line.begins_with("~ "):
			...
			for p in range(line_number - 1, -1, -1):
				...
				if line_indent < expected_indent:
					return dialog[str(p)].next_id_after

	return DialogConstants.ID_END_CONVERSATION


func find_next_line_after_responses(line_number: int, all_lines: Array, dialog: Dictionary, parent_stack: Array) -> String:
	var line = all_lines[line_number]
	var expected_indent = get_indent(line)
	...
	for i in range(line_number + 1, all_lines.size()):
		...
		# Another option
			...
			if indent == expected_indent:
				...
				return dialog[str(previous_parent)].next_id_after
...
func extract_mutation(line: String) -> Dictionary:
	...
	if found.names.has("function"):
		...
		if expression.size() > 0 and expression[0].get("type") == DialogConstants.TYPE_ERROR:
	...
	# Otherwise we are setting a variable so expressionise its new value
		...
		if expression[0].get("type") == DialogConstants.TYPE_ERROR:
...
func extract_dialog_replacements(text: String) -> Array:
...
func extract_goto(line: String, titles: Dictionary) -> String:
	...
	if found == null: return DialogConstants.ID_ERROR
	...
	if title == "END":
		...
		return DialogConstants.ID_ERROR
...
func build_token_tree_error(message: String) -> Array:
	return [{ "type": DialogConstants.TOKEN_ERROR, "value": message}]
...
func build_token_tree(tokens: Array, expected_close_token: String = "") -> Array:
	...
	while tokens.size() > 0 and limit < 1000:
		...
		match token.type:
			DialogConstants.TOKEN_FUNCTION:
				var sub_tree = build_token_tree(tokens, DialogConstants.TOKEN_PARENS_CLOSE)

				if sub_tree[0].size() > 0 and sub_tree[0][0].get("type") == DialogConstants.TOKEN_ERROR:
					return [build_token_tree_error(sub_tree[0][0].get("value")), tokens]

				tree.append({
					"type": DialogConstants.TOKEN_FUNCTION,
			...
			DialogConstants.TOKEN_DICTIONARY_REFERENCE:
				var sub_tree = build_token_tree(tokens, DialogConstants.TOKEN_BRACKET_CLOSE)
				...
				tree.append({
					"type": DialogConstants.TOKEN_DICTIONARY_REFERENCE,
					# Consume the trailing "["
			...
			DialogConstants.TOKEN_BRACE_OPEN:
				var sub_tree = build_token_tree(tokens, DialogConstants.TOKEN_BRACE_CLOSE)

				if sub_tree[0].size() > 0 and sub_tree[0][0].get("type") == DialogConstants.TOKEN_ERROR:
					return [build_token_tree_error(sub_tree[0][0].get("value")), tokens]

				tree.append({
					"type": DialogConstants.TOKEN_DICTIONARY,
			...
			DialogConstants.TOKEN_BRACKET_OPEN:
				...
				tree.append({
					"type": DialogConstants.TOKEN_ARRAY,
					"value": tokens_to_list(sub_tree[0])
				})
				tokens = sub_tree[1]
			...
			DialogConstants.TOKEN_PARENS_OPEN:
				...
				if sub_tree[0][0].get("type") == DialogConstants.TOKEN_ERROR:
					return [build_token_tree_error(sub_tree[0][0].get("value")), tokens]

				tree.append({
					"type": DialogConstants.TOKEN_GROUP,
					"value": sub_tree[0]
				})
				tokens = sub_tree[1]
			...
			DialogConstants.TOKEN_NOT:
				# Double nots negate each other
				if tokens.size() > 0 and tokens.front().get("type") == DialogConstants.TOKEN_NOT:
					tokens.pop_front()
				else:
					tree.append({
						"type": token.get("type")
			...
			DialogConstants.TOKEN_COMPARISON, \
			DialogConstants.TOKEN_ASSIGNMENT, \
			DialogConstants.TOKEN_OPERATOR, \
			DialogConstants.TOKEN_AND_OR, \
			DialogConstants.TOKEN_VARIABLE: \
				tree.append({
					"type": token.get("type"),
			...
			DialogConstants.TOKEN_BOOL:
				tree.append({
					"type": token.get("type"),
					"value": token.get("value").to_lower() == "true"
				})

			DialogConstants.TOKEN_NUMBER:
				tree.append({
					"type": token.get("type"),
...
func tokens_to_list(tokens: Array) -> Array:
	...
	var current_item = []
	for token in tokens:
		if token.get("type") == DialogConstants.TOKEN_COMMA:
...
func tokens_to_dictionary(tokens: Array) -> Dictionary:
	...
	for i in range(0, tokens.size()):
		if tokens[i].get("type") == DialogConstants.TOKEN_COLON:

...
onready var settings: DialogSettings = get_node(_settings) as DialogSettings
...
var dialog_manager_config := ConfigFile.new()
...
func _on_SettingsDialog_about_to_show():
	...
	missing_translations_button.pressed = settings.get_editor_value(
		"missing_translations_are_errors", false
	)
	wrap_button.pressed = settings.get_editor_value("wrap_lines", false)
	include_all_responses_button.pressed = settings.get_runtime_value(

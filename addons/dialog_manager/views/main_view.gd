...
onready var new_dialog_dialog := $NewDialogDialog
onready var open_dialog_dialog := $OpenDialogDialog
onready var invalid_dialog_dialog := $InvalidDialogDialog
...
var plugin
var current_resource: DialogResource
...
func _ready() -> void:
	...
	var err = config.load("res://addons/dialog_manager/plugin.cfg")
	if err == OK:
...
func set_resource(value: DialogResource) -> void:
...
func open_resource(resource: DialogResource) -> void:
...
func open_resource_from_path(path: String) -> void:
	...
	if resource is DialogResource:
		...
		invalid_dialog_dialog.popup_centered()


func apply_upgrades(resource: DialogResource) -> void:
	if resource == null:
		return
	if not resource is DialogResource:
		return
	...
	resource.set("syntax_version", DialogConstants.SYNTAX_VERSION)
...
func parse(force_show_errors: bool = false) -> void:
	if current_resource == null:
		return
	if not has_changed and not force_show_errors:
		return
	...
	if (
		force_show_errors
		or settings.get_editor_value("check_for_errors")
		or error_list.errors.size() > 0
	):
...
func generate_translations_keys() -> void:
	...
	for i in lines.size():
		...
		if (
			l.begins_with("if ")
			or l.begins_with("elif ")
			or l.begins_with("else")
			or l.begins_with("endif")
		):
			continue
		if l.begins_with("~ "):
			continue
		if l.begins_with("do ") or l.begins_with("set "):
			continue
		if l.begins_with("=>"):
			continue
...
func save_translations(path: String) -> void:
	...
	var dialog = parser.parse(editor.text).get("lines")

	# Make a list of stuff that needs to go into the file
	var lines_to_save = []
	for key in dialog.keys():
		var line: Dictionary = dialog.get(key)

		if not line.get("type") in [DialogConstants.TYPE_DIALOG, DialogConstants.TYPE_RESPONSE]:
			continue
		if line.get("text") in known_keys:
			continue
		...
		if existing_csv.has(line.get("translation_key")):
			...
			lines_to_save.append(
				PoolStringArray([line.get("translation_key"), line.get("text")] + commas)
			)
...
func _on_open_menu_index_pressed(index):
	...
	match item:
		"Open...":
			open_dialog_dialog.popup_centered()
		"Clear recent files":
...
func _on_translation_menu_id_pressed(id):
	match id:
		...
		1:
			...
			var path = (
				settings.get_editor_value(
					"last_csv_path", current_resource.resource_path.get_base_dir()
				)
				+ "/"
				+ filename
			)
...
func _on_NewButton_pressed():
	new_dialog_dialog.popup_centered()


func _on_NewDialogDialog_file_selected(path):
...
func _on_OpenDialogDialog_file_selected(path):
	open_resource_from_path(path)


func _on_OpenDialogDialog_confirmed():
	open_resource_from_path(open_dialog_dialog.current_path)
...
func _on_HelpButton_pressed():
	OS.shell_open("https://github.com/nathanhoad/godot_dialog_manager")
...
func _on_RunButton_pressed():
	if settings.has_editor_value("run_title"):
		...
		plugin.get_editor_interface().play_custom_scene(
			"res://addons/dialog_manager/views/test_scene.tscn"
		)

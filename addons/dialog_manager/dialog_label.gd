		...
		if percent_visible < 1:
			...
			if Input.is_action_just_pressed(skip_action):
				...
				for i in range(index, get_total_character_count()):
					dialog.mutate_inline_mutations(i)
...
func reset_height() -> void:
	...
	size_check_label.bbcode_text = dialog.dialog

	# Give the size check a chance to resize
	yield(get_tree(), "idle_frame")

	# Resize our dialog label with the new size hint
...
func type_next(delta: float, seconds_needed: float) -> void:
	if last_mutation_index != index:
		...
		dialog.mutate_inline_mutations(index)

	if last_wait_index != index and dialog.get_pause(index) > 0:
		last_wait_index = index
		waiting_seconds += dialog.get_pause(index)
		emit_signal("paused", dialog.get_pause(index))
	else:
		percent_visible += percent_per_index
		index += 1
		seconds_needed += seconds_per_step * (1.0 / dialog.get_speed(index))
		if seconds_needed > delta:
			waiting_seconds += seconds_needed
			if index < text.length():
				emit_signal("spoke", text[index - 1], dialog.get_speed(index))
		else:
			type_next(delta, seconds_needed)


func type_out() -> void:
	bbcode_text = dialog.dialog
	percent_visible = 0
	index = 0
	has_finished = false
	waiting_seconds = 0
